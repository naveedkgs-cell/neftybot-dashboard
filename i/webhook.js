export default async function handler(req, res) {
  if (req.method !== 'POST') {
    return res.status(200).json({ status: 'NeftyBot V3 Webhook Ready - Use POST' });
  }
  ...
  try {
    ...
    return res.status(200).json({ status: 'Order Sent', action: orderAction, symbol: orderSymbol, dhanResponse: data });
